...
export default function AlgorithmSelector({
  ...
  return (
    <div className="bg-white p-4 rounded-2xl border border-green-50 shadow-sm flex flex-col gap-4">
      {/* 1. Header */}
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          <div className="flex items-center justify-between w-full">
            ...
            <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full ${
              selectedAlgo === 'dijkstra' ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'
            }`}>
              $O(E \log V)$
            </span>
          </div>
          <p className={`text-[10px] leading-relaxed font-medium ${selectedAlgo === 'dijkstra' ? 'text-green-100' : 'text-slate-500'}`}>
        ...
        >
          <div className="flex items-center justify-between w-full">
            ...
            <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full ${
              selectedAlgo === 'prim' ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'
            }`}>
              $O(E \log V)$
            </span>
          </div>
          <p className={`text-[10px] leading-relaxed font-medium ${selectedAlgo === 'prim' ? 'text-green-100' : 'text-slate-500'}`}>
        ...
        >
          <div className="flex items-center justify-between w-full">
            ...
            <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full ${
              selectedAlgo === 'tsp' ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'
            }`}>
              $O(N!)$ vs $O(N^2 2^N)$
            </span>
          </div>
          <p className={`text-[10px] leading-relaxed font-medium ${selectedAlgo === 'tsp' ? 'text-green-100' : 'text-slate-500'}`}>
      ...
      {selectedAlgo === 'dijkstra' && (!hasHub || !hasTarget) && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2 animate-pulse">
          ...
          <div className="text-[10px] leading-relaxed text-amber-700 font-semibold">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-1.5 bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg transition-colors"
              title="Previous Step"
            >
            ...
            <button
              ...
              className={`p-2 rounded-xl text-white shadow-sm transition-all flex items-center justify-center ${
                isPlaying
                  ...
                  : 'bg-green-600 hover:bg-green-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none'
              }`}
            >
            ...
            <button
              onClick={onReset}
              className="p-1.5 bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-600 rounded-lg transition-colors"
          ...
          <div className="flex items-center gap-2 max-w-[120px] w-full">
            ...
            />
            <span className="text-[9px] font-bold text-slate-400 whitespace-nowrap min-w-[32px] text-right">
              {playbackSpeed}ms
            </span>
          </div>
        </div>
        ...
        {totalSteps > 0 && (
          <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 flex flex-col gap-2">
            <div className="flex items-center justify-between text-[10px] font-extrabold text-slate-500">
              <span className="uppercase text-green-500">Solver Stepper</span>
              <span>
            ...
            {/* Stepper Description */}
            <p className="text-[10px] leading-relaxed text-slate-600 font-semibold mt-1">
