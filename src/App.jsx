...
export default function App() {
        ...
        if (n > 20) {
          ...
          if (n <= 6 && bruteRes && bruteRes.steps.length > 0) {
            const tspSteps = bruteRes.steps.map((step) => ({
              ...
              description:
                `[Permutation Search] Checked path permutation #${step.evaluatedCount}: ${step.tour
                  .map(
                    (id) =>
                      nodes.find((n) => n.id === id)?.label?.trim() ||
                      `Location ${id}`,
                  )
                  .join(" ➔ ")} (Cost: ${step.cost.toFixed(1)} km). ` +
                (step.isBetter
                  ? `★ Found shorter delivery tour!`
                  : `Keep searching...`),
            }));
  ...
  useEffect(() => {
    ...
    if (selectedAlgo === "dijkstra") {
      ...
      if (visited)
        visited.forEach((id) => (newHighlightedNodes[id] = "visited"));
      ...
      if (highlightEdges)
        highlightEdges.forEach((e) => (newHighlightedEdges[e.key] = "active"));

      if (parents && type !== "done") {
        Object.entries(parents).forEach(([childId, parentId]) => {
          if (parentId !== null) {
            const edgeKey = [
              Math.min(childId, parentId),
              Math.max(childId, parentId),
            ].join("-");
      ...
      if (type === "search" && tour) {
        for (let i = 0; i < tour.length - 1; i++) {
          const edgeKey = [
            Math.min(tour[i], tour[i + 1]),
            Math.max(tour[i], tour[i + 1]),
          ].join("-");
          newHighlightedEdges[edgeKey] = "active";
        }
        ...
        for (let i = 0; i < expandedTour.length - 1; i++) {
          const edgeKey = [
            Math.min(expandedTour[i], expandedTour[i + 1]),
            Math.max(expandedTour[i], expandedTour[i + 1]),
          ].join("-");
          newHighlightedEdges[edgeKey] = "solution";
        }
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-black text-slate-100 flex flex-col antialiased">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1 bg-neutral-950 border border-neutral-900 px-2 py-1 rounded-xl">
            ...
            <button
              ...
              disabled={
                isPlaying || currentStepIndex <= 0 || algoSteps.length === 0
              }
            ...
            <button
              ...
              disabled={
                isPlaying ||
                currentStepIndex >= algoSteps.length - 1 ||
                algoSteps.length === 0
              }
              className="p-1.5 rounded-lg hover:bg-neutral-900 text-neutral-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              title="Step Forward"
          ...
          <button
            ...
            title={
              isSidebarOpen ? "Hide Analytics Panel" : "Show Analytics Panel"
            }
          >
      ...
      <div className="flex-1 w-full flex overflow-hidden relative">
        ...
        <div
          className={`h-full bg-[#0a0a0a]/90 backdrop-blur-lg border-l border-neutral-900 shadow-2xl overflow-y-auto p-5 z-20 shrink-0 custom-scrollbar transition-all duration-300 ${
            isSidebarOpen
              ? "w-[360px] opacity-100"
          ...
          >
            {executionLog.length === 0 ? (
              <div className="text-neutral-500 italic text-center my-auto">
                CONSOLE STANDBY — Run an optimization solver to view
              ...
              executionLog.map((log, index) => (
                ...
                >
                  <span className="text-green-400 font-bold shrink-0 w-8 text-right">
                    [{index + 1}]
                  </span>
      ...
      <div
        className={`fixed left-[20px] flex flex-col gap-2 z-50 pointer-events-none transition-all duration-300 ${isConsoleOpen ? "bottom-[300px]" : "bottom-16"}`}
      >
        {toasts.map((t) => (
          <div
            key={t.id}
